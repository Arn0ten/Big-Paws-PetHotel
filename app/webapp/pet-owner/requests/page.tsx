...
export default function PetOwnerRequestsPage() {
  ...
  useEffect(() => {
    ...
    if (
      urlTab &&
      ["pending", "in-progress", "completed", "rejected"].includes(urlTab)
    ) {
      setActiveTab(urlTab);
    }
  ...
  const renderRequestsList = (status: string): React.JSX.Element => {
          ...
          >
            <Card className="overflow-hidden">
              <CardContent className="p-0">
                <Link
                  href={`/webapp/pet-owner/requests/${request.id}?tab=${activeTab}`}
                  className="block"
                >
  ...
  const handleTabChange = (value: string) => {
    setActiveTab(value);
    // Update URL with the new active tab without full page reload
      ...
      <div className="space-y-6">
        ...
        <Alert className="bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800 mb-4">
          ...
          <AlertDescription className="text-blue-700 dark:text-blue-400">
            ...
            <Badge className="bg-yellow-600 text-white font-normal">
              Pending
            </Badge>{" "}
            status. Once a request is{" "}
            <Badge className="bg-green-600 text-white font-normal">
              In Progress
            </Badge>
