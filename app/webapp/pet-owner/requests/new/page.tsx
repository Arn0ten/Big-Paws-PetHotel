...
/**
 * Pet Owner Request Creation Page
 *
 * This component allows pet owners to create new service requests for their pets.
 *
 * =====================================================================
 ...
 *    - GET /api/pets?boarding=true - To fetch pets currently boarding
 *    - POST /api/requests - To create a new request
 *
 * 2. Data Models:
 *    - Pet: See interface in app/webapp/types/index.ts
 *    - Request: See interface in app/webapp/types/index.ts
 *
 * 3. Integration Points:
 *    - Replace mock data functions with actual API calls (marked with BACKEND INTEGRATION comments)
 *    - Implement proper error handling and loading states
 *    - Add authentication checks and user context
 *
 * 4. Form Validation:
 *    - Server-side validation should match client-side validation rules
...
export default function EnhancedRequestCreationPage() {
   ...
   *
   * API Endpoint: GET /api/pets?boarding=true
   *
   * Expected Response:
   * Array of Pet objects with boarding information
   *
  ...
  useEffect(() => {
    const fetchPets = async () => {
      try {
        ...
        // Check for petId in URL query parameters
        const params = new URLSearchParams(window.location.search);
        const petId = params.get("petId");

        if (petId) {
          // If petId is provided, pre-select the pet and move to step 2
          setSelectedPet(petId);
          setActiveStep(2); // Skip to step 2 (request type selection)
        }
      } catch (error) {
        console.error("Error fetching pets:", error);
        setErrorMessage("Failed to load your pets. Please try again later.");
   ...
   *
   * BACKEND INTEGRATION:
   * This function should be updated to make an actual API call to create a new request.
   *
   ...
   *   status: string,         // Initial status (typically "pending" or "new")
   *
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Generate a title if not provided (since we removed the title field)
    const generatedTitle = `${petName} - ${requestTypes.find((type) => type.id === selectedRequestType)?.name}`;
    ...
    try {
      ...
      // BACKEND INTEGRATION: Remove this when implementing real API
      ...
      // } else {
      //   setErrorMessage("Failed to create your request. Please try again.");
      // }
        ...
        {[1, 2, 3].map((step) => (
          <div key={step} className="flex flex-col items-center">
            ...
            <span className="text-xs mt-1 text-muted-foreground">
              {step === 1 ? "Pet" : step === 2 ? "Type" : "Details"}
            </span>
          </div>
        ))}
      </div>
  ...
  const renderCurrentStep = () => {
    switch (activeStep) {
      case 1:
        return renderStep2(); // Now step 1 is pet selection (previously step 2)
      case 2:
        return renderStep1(); // Now step 2 is request type (previously step 1)
      case 3:
        return renderStep3();
      default:
        return null;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Desktop buttons */}
      <div className="hidden md:flex gap-4 justify-end mb-6">
        {activeStep > 1 ? (
      ...
      {/* Mobile floating buttons - simplified with just the buttons */}
      <div className="md:hidden fixed bottom-20 left-4 right-4 z-10 flex gap-2">
        {activeStep > 1 ? (
          <Button
            type="button"
            variant="outline"
