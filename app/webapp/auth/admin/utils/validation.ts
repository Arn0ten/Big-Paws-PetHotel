...
export function validatePassword(password: string, isAdmin = true): { isValid: boolean; error?: string } {
  if (!password) {
  ...
  if (isAdmin && password.length < 10) {
    return { isValid: false, error: "Admin password must be at least 10 characters long" }
  ...
  if (isAdmin) {
    ...
    if (specialChars.length < 2) {
      return { isValid: false, error: "Admin password must contain at least two special characters" }
    }
  }
