...
export default function PetOwnersPage() {
  ...
  const handleAddPet = async (petData: Partial<Pet>) => {
    ...
    try {
      ...
      const selectedOwner =
        petOwners.find((o) => o.id === selectedOwnerId) || null;
      navigateTo({ type: "details", ownerId: selectedOwnerId });
                ...
                <CardHeader>
                  <div className="flex flex-col space-y-4">
                    <div className="flex items-center">
                      <FaUserCheck className="mr-2 h-5 w-5 text-primary" />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-md border overflow-hidden">
                      <PetOwnerTable
                        ...
                        onPetClick={(pet) =>
                          navigateTo({ type: "petDetails", petId: pet.id })
                        }
                        onAddPet={(id) =>
                          navigateTo({ type: "addPet", ownerId: id })
                        }
                        onBoardPet={(id) =>
                          navigateTo({ type: "board", ownerId: id })
                        }
                        onEditOwner={(id) =>
                          navigateTo({ type: "edit", ownerId: id })
                        }
                        onDeleteOwner={(id) => {
                        ...
                        onRowClick={(id) =>
                          navigateTo({ type: "details", ownerId: id })
                        }
                        itemsPerPage={ITEMS_PER_PAGE}
      ...
      case "details":
        return (
          <PetOwnerDetailsView
            ...
            onEdit={() =>
              selectedOwner &&
              navigateTo({ type: "edit", ownerId: selectedOwner.id })
            }
            onDelete={() => {
              if (selectedOwner) {
                showConfirmation(
            ...
            onAddPet={() =>
              selectedOwner &&
              navigateTo({ type: "addPet", ownerId: selectedOwner.id })
            }
            onBoardPet={() =>
              selectedOwner &&
              navigateTo({ type: "board", ownerId: selectedOwner.id })
            }
            onPetClick={(pet) =>
              navigateTo({ type: "petDetails", petId: pet.id })
            }
      ...
      case "petDetails":
        return (
          <PetDetailsView
            ...
            onEdit={() =>
              selectedPet &&
              navigateTo({ type: "editPet", petId: selectedPet.id })
            }
            onBoard={() =>
              selectedPet &&
              selectedOwner &&
