...
import {useCreateBoarding} from "@/webapp/admin/pet-owners/hooks";
...
// Boarding type
type BoardingType = "DAYCARE" | "LONG_STAY";
...
export default function BoardPetsView({
    ...
    const {createBoarding, isLoading, error, currentBoarding, resetError} = useCreateBoarding();  // Correct way to destructure


    // Handle form submission
    const handleSubmit = async (e: React.FormEvent, d: BoardingDetails) => {
        e.preventDefault();
        ...
        try {
            const boardingData = toPetBoardingRequest(d); // Convert formData to PetBoardingRequestDTO
            const newBoarding = await createBoarding(boardingData); // Call the createBoarding function from the hook
    ...
    function toPetBoardingRequest(formData: BoardingDetails): PetBoardingRequestDTO {
        const petId = formData.petIds[0]; // Pick the first pet
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                <div
                                    className={`flex items-center space-x-3 p-3 rounded-lg border transition-colors ${formData.type === "LONG_STAY"
                                        ? "border-primary bg-primary/10 shadow-sm"
                                        : "hover:bg-muted/50 border-border"
                                    }`}
                                >
