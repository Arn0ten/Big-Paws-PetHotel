...
interface UnifiedBoardPetDialogProps {
  // For pet owner mode
  owner?: any
  // For single pet mode
  pet?: any
  ...
  onSubmit: (details: any) => Promise<boolean>
}

export function UnifiedBoardPetDialog({ owner, pet, isOpen, onOpenChange, onSubmit }: UnifiedBoardPetDialogProps) {
  ...
  // Determine if we're in single pet mode or pet owner mode
  const isSinglePetMode = !!pet

  // Reset form when dialog opens or owner/pet changes
  useEffect(() => {
    if (isOpen) {
      setStep(1)
      setBoardingDetails({
        petIds: isSinglePetMode ? [pet.id] : [],
        type: "Daycare",
        startDate: new Date(),
        endDate: new Date(),
        startTime: "09:00",
  ...
  }, [isOpen, owner, pet, isSinglePetMode])

  // Get selected pets
  const selectedPets = isSinglePetMode
    ? [pet]
    : owner?.pets.filter((pet) => boardingDetails.petIds.includes(pet.id)) || []

  // Handle pet selection (only in pet owner mode)
  ...
  const canProceedToStep2 = boardingDetails.petIds.length > 0 || isSinglePetMode
  ...
  // Get the title and description based on mode
  const dialogTitle = isSinglePetMode ? `Board ${pet.name}` : `Board ${owner?.name}'s Pets`

  const dialogDescription = isSinglePetMode
    ? `Create a boarding reservation for ${pet.name}`
    : `Create a boarding reservation for ${owner?.name}'s pets`
        ...
        <div className="px-6">
          <AnimatePresence mode="wait">
            {step === 1 && (
              ...
              >
                {isSinglePetMode ? (
                  // Single pet mode - show pet details
                  <>
                      ...
                      <div>
                        ...
                        <div className="flex items-center text-sm text-muted-foreground">
                          <Badge
                            variant="outline"
                            className={
                              pet.type === "Dog" ? "bg-primary/10 text-primary" : "bg-secondary/10 text-secondary"
                            }
                          >
                            {pet.type}
                          </Badge>
                              ...
                              <Tooltip>
                                ...
                                <TooltipContent side="right" className="max-w-xs">
                                  <div className="space-y-1">
                                    ...
                                    <p className="text-xs">
                                      {pet.type} • {pet.breed} • {pet.size}
                                    </p>
                                    <p className="text-xs">
                                      Age: {pet.age} {pet.age === 1 ? "year" : "years"}
                                    </p>
                                    {pet.notes && <p className="text-xs">Notes: {pet.notes}</p>}
                                  </div>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
