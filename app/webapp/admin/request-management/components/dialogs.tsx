...
import { Loader2, AlertTriangle, Bell, DollarSign } from "lucide-react"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`${isMobile ? "max-w-[95%]" : "sm:max-w-md"}`}>
        <DialogHeader>
          ...
          <DialogDescription>
            This will move the request back to the "New Requests" tab. Please provide a reason for this change that will
            be visible to the pet owner.
          </DialogDescription>
        </DialogHeader>
...
export function BoardingDetailsDialog({ open, onOpenChange, boardingDetails }: BoardingDetailsDialogProps) {
  const isMobile = useMediaQuery("(max-width: 640px)")
        ...
        <div className="space-y-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="text-base font-medium mt-1">{boardingDetails.pet.name}</div>
            </div>

            <div>
              <span className="text-xs uppercase tracking-wide text-muted-foreground font-medium">Owner</span>
              <div className="text-base font-medium mt-1">{boardingDetails.owner.name}</div>
            </div>
          </div>

          <div>
            <span className="text-xs uppercase tracking-wide text-muted-foreground font-medium">Original Total</span>
            <div className="text-base font-medium mt-1">
              {formatCurrency(boardingDetails.totalPrice - (boardingDetails.additionalCharges || 0))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 p-3 bg-green-50 border border-green-100 rounded-md text-green-700 dark:bg-green-950/20 dark:border-green-800 dark:text-green-300 flex items-center">
              ...
              <span className="text-lg font-medium">{formatCurrency(boardingDetails.additionalCharges || 0)}</span>
              <span className="ml-2 text-xs text-muted-foreground">({boardingDetails.additionalChargesReason})</span>
            </div>
          </div>
