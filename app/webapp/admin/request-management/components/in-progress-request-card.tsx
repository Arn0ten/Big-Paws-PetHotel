...
export function InProgressRequestCard({ request, onProcess, onViewDetails, onUndoAccept }: InProgressRequestCardProps) {
    ...
    >
      ...
      >
        ...
        <CardContent className="p-4 pt-2 flex-grow overflow-hidden">
          ...
          <div className="mt-3">
            <span className="text-xs uppercase tracking-wide text-muted-foreground font-medium">Approved</span>
            <div className="text-sm font-medium mt-0.5">{formatDate(request.approvedAt || request.createdAt)}</div>
          </div>
        </CardContent>
        <CardFooter className="p-4 pt-0 mt-auto flex flex-col sm:flex-row gap-2">
          ...
          <Button
            ...
            disabled={isUndoing}
          >
            {isUndoing ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4" />}
          </Button>
        </CardFooter>
      </Card>
    </motion.div>
