...
export default function EnhancedRequestDialog({
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      tempVideo.onloadedmetadata = () => {
        ...
        if (tempVideo.duration > 60) {
          alert("Video exceeds maximum duration of 60 seconds. Please select a shorter video.")
  ...
  const formattedCurrentEndDate = request.currentEndDate ? `${formatDate(request.currentEndDate)}` : "Not specified"
  ...
  const getSteps = () => {
    switch (request.type) {
      ...
      case "grooming":
        return ["Review Request", "Confirm Service", "Upload Photos", "Add Notes", "Complete"]
      case "boarding-extension":
  ...
  const canGoNext = () => {
    ...
    if (request.type === "photo" && currentStep === 1 && selectedFiles.length === 0) {
      return false
    }
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto p-0 max-w-[95vw] w-full overflow-x-hidden">
        <DialogHeader className={`p-6 pb-4 border-b ${getRequestTypeBgClass()}`}>
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-full bg-white dark:bg-gray-800 ${getRequestTypeColorClass()}`}>
              {getRequestTypeIcon()}
            </div>
            <div>
              <DialogTitle className="text-xl font-semibold">
                Process {request.petName}'s {request.type.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())}{" "}
                Request
              </DialogTitle>
              <p className="text-sm text-muted-foreground mt-1">
                Submitted by {request.petOwnerName} on {formatDate(request.createdAt)}
              </p>
            </div>
          </div>
        </DialogHeader>
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Upload up to 5 photos of {request.petName} to share with the pet owner.
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-4">
                  <PhotoUpload
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>Upload a video of {request.petName} to share with the pet owner.</CardDescription>
                </CardHeader>
                <CardContent className="pt-4">
                  <VideoUpload
            ...
            <div className="space-y-4">
              <Card className="border-green-200 dark:border-green-800">
                <CardHeader className="bg-green-50 dark:bg-green-950/20 pb-2">
                  ...
                  <CardDescription>Review and confirm the grooming service for {request.petName}.</CardDescription>
                </CardHeader>
                <CardContent className="pt-4 space-y-4">
                  ...
                  <div className="p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-md">
                    <p className="text-xs text-amber-700 dark:text-amber-400">
                      Note: Grooming service details are predefined and cannot be modified. The price is automatically
            ...
            <div className="space-y-4">
              <Card className="border-amber-200 dark:border-amber-800">
                <CardHeader className="bg-amber-50 dark:bg-amber-950/20 pb-2">
                  ...
                  <CardDescription>Review and confirm the boarding extension for {request.petName}.</CardDescription>
                </CardHeader>
                <CardContent className="pt-4 space-y-4">
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <Label className="text-sm font-medium">Extension Requested:</Label>
                    ...
                    {isHourExtension ? (
                      ...
                      <>
                        <div className="space-y-1">
                          <Label htmlFor="extension-date" className="text-sm font-medium">
                            New End Date:
                          </Label>
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>
                    Upload up to 2 photos of {request.petName} after grooming to share with the pet owner.
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-4">
                  <PhotoUpload
            ...
            <div className="space-y-6">
              <Card className={`border ${getRequestTypeBorderClass()}`}>
                <CardHeader className={`${getRequestTypeBgClass()} pb-2`}>
                  ...
                  <CardDescription>Review the information below before completing the request.</CardDescription>
                </CardHeader>
                <CardContent className="pt-4 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                    ...
                    {request.type === "photo" && (
                      <div>
                        ...
                        <div className="mt-1 text-base font-medium">{selectedFiles.length} photos</div>
                      </div>
                    )}

                    {request.type === "video" && (
                      <div>
                        <Label className="text-xs text-muted-foreground uppercase tracking-wider">Video Uploaded</Label>
                        <div className="mt-1 text-base font-medium">{selectedFiles.length > 0 ? "Yes" : "No"}</div>
                      </div>
                    )}
                    ...
                    {request.type === "boarding-extension" && (
                      <>
                        <div>
                          <Label className="text-xs text-muted-foreground uppercase tracking-wider">Extension</Label>
                          <div className="mt-1 text-base font-medium text-amber-700 dark:text-amber-400">
                            {formattedExtensionRequested}
                          </div>
                        </div>
                        <div>
                          <Label className="text-xs text-muted-foreground uppercase tracking-wider">New End Date</Label>
                          <div className="mt-1 text-base font-medium">{formattedNewEndDate}</div>
                        </div>
                        <div>
                          <Label className="text-xs text-muted-foreground uppercase tracking-wider">
                            Additional Cost
                  ...
                  {request.type === "video" && videoFile && (
                    <div className="col-span-2 mt-2">
                      ...
                      >
                        <video
                          ref={videoRef}
                          src={audioMerged && mergedVideoUrl ? mergedVideoUrl : videoPreviewUrl || undefined}
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          >
                            {isVideoPlaying ? <Pause className="h-6 w-6" /> : <Play className="h-6 w-6 fill-white" />}
                          </button>
                        </div>
                  ...
                  {processingNotes && (
                    <div>
                      <Label className="text-xs text-muted-foreground uppercase tracking-wider">Processing Notes</Label>
                      <div className="mt-1 p-3 bg-muted/30 rounded-md text-base whitespace-pre-wrap">
                        {processingNotes}
                      </div>
                    </div>
                  ...
                  <div className="p-3 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-md">
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                      Completing this request will notify the pet owner and update the status to "Completed".
                      {(request.type === "grooming" || request.type === "boarding-extension") &&
        ...
        <Dialog open={showVideoFullscreen} onOpenChange={setShowVideoFullscreen}>
          <DialogContent className="p-0 max-w-[100vw] h-[100vh] border-none bg-transparent shadow-none">
            <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center">
              {/* Close button */}
              <button
        ...
        <DialogFooter className="p-4 border-t flex flex-col sm:flex-row gap-2">
          {currentStep > 0 && (
            <Button variant="outline" onClick={() => setCurrentStep(currentStep - 1)} className="flex items-center">
          ...
          {currentStep === steps.length - 1 && (
            <Button onClick={handleComplete} disabled={isProcessing || !canComplete()} className="flex items-center">
              {isProcessing ? (
                <>
