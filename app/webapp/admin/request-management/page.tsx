...
export default function RequestManagementPage() {
  ...
  // Add a function to handle tab switching with animation
  const switchToTab = (tabValue: string) => {
    // Add a small animation effect when switching tabs
    const tabElement = document.querySelector(`[data-value="${tabValue}"]`);
    if (tabElement) {
      tabElement.classList.add("tab-switch-pulse");
      setTimeout(() => {
        tabElement.classList.remove("tab-switch-pulse");
  ...
  // Add CSS for tab switching animation
  useEffect(() => {
    const style = document.createElement("style");
    style.innerHTML = `
      .tab-switch-pulse {
        animation: tab-pulse 0.6s ease-in-out;
      }
  ...
  // Modified handleCompleteRequest function to skip success dialog and navigate directly
  const handleCompleteRequest = () => {
    if (!selectedRequest) return;

    setIsProcessing(true);

    // Simulate API call - reduced timeout for faster response
    setTimeout(() => {
      ...
      if (
        ...
        if (boardingDetails) {
          ...
          // Show boarding details dialog after a short delay
          setTimeout(() => {
      ...
      toast({
        ...
        description: `The ${getRequestTypeLabel(selectedRequest.type).toLowerCase()} for ${selectedRequest.petName} has been completed.`,
        duration: 3000,
      });
      ...
      // Immediately switch to the completed tab
  ...
  const handleSearch = (query: string) => {
    ...
    // Simulate search delay and refresh data - reduced timeout for faster response
    setTimeout(() => {
  ...
  const handleRefresh = () => {
    ...
    // Simulate refreshing data - reduced timeout for faster response
    setTimeout(() => {
      resetRequests();
      setIsLoading(false);
          ...
          <div className="px-4 pt-4">
            <TabsList className="w-full grid grid-cols-2 h-14 p-1 bg-muted/70 dark:bg-muted/60 rounded-lg shadow-sm">
              ...
              >
                ...
                {hasNewCompletedRequests && activeTab !== "completed" && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
