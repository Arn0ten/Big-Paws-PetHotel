...
import {useBoardingOperations, useFetchActiveBoarding} from './hooks';
...
export default function BoardingManagementPage() {
    // ????
    const [boardingOrders, setBoardingOrders] = useState<BoardingDTO[]>([])
    const [filteredOrders, setFilteredOrders] = useState<BoardingDTO[]>([])
    ...
    // REAL DATA
    const [boardingData, setBoardingData] = useState<BoardingDTO[]>([])
    const [filteredData, setFilteredData] = useState<BoardingDTO[]>([])
    ...
    const {fetchActiveBoarding} = useFetchActiveBoarding();

    useEffect(() => {
        console.log("Fetching boarding data...");
        fetchActiveBoarding()
            .then((data: BoardingDTO[]) => {
    ...
    useEffect(() => {
        console.log("Updated boardingData:", boardingData);
    ...
    const fetchBoardingOrders = useCallback(async () => {
        try {
            ...
            const ordersWithOverdueStatus = checkOverduePickups(boardingData)
    ...
    useEffect(() => {
        ...
        // Apply tab filter
        if (activeTab === "active") {
            result = result.filter((order) => !order.overdue && order.boardingStatus === "BOARDING")
        } else if (activeTab === "completed") {
            result = result.filter((order) => order.boardingStatus === "DONE_BOARDING")
        } else if (activeTab === "overdue") {
            result = result.filter((order) => order.overdue && order.boardingStatus != "RELEASED")
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Stats Cards */}
            <StatsCards boardingOrders={boardingData} isLoading={isTableLoading} onCardClick={handleCardClick}/>
            ...
            <div className="overflow-x-auto pb-2">
                <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
                    <div className="overflow-x-auto pb-2">
                        <TabsList
                            className="w-full sm:w-auto flex flex-nowrap justify-start overflow-x-auto bg-muted/30 p-1">
                            <TabsTrigger
                                value="all"
